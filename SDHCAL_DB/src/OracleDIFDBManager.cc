...
#include <map>
OracleDIFDBManager::OracleDIFDBManager(std::string sdifs,std::string setup,std::string xmlfile) : DIFDBManager(sdifs),theOracleSetupName_(setup),theXMLFile_(xmlfile)
{}
void OracleDIFDBManager::initialize()
{
 ...
 if (theXMLFile_.compare("NONE")==0)
 ...
 else
   {
     ...
     State* s= State::createStateFromXML(theXMLFile_,theXMLFile_);
     theOracleSetup_->addState(s);
   }
}

void OracleDIFDBManager::LoadDIFDefaultParameters(uint32_t difid, UsbDIFSettings* v)
{
...
#undef BUGINDB
uint32_t OracleDIFDBManager::LoadDIFHardrocV2Parameters(uint32_t difid_request,SingleHardrocV2ConfigurationFrame* ConfigHR2)
{
  uint8_t thr=0;
	uint8_t thrl1=0;
	uint8_t thrl2=0;
...
uint32_t OracleDIFDBManager::LoadDIFMicrorocParameters(uint32_t difid, SingleHardrocV2ConfigurationFrame* ConfigMR)
{
  uint8_t tmr=0;  // thr
	uint8_t tmrl1=0;
	uint8_t tmrl2=0;
...
void OracleDIFDBManager::storeHardRoc2Configuration(Asic* itHR2,unsigned char* ConfigHR2)
{
  unsigned long long tmask0;
  unsigned long long tmask1;
  unsigned long long tmask2;
...
void OracleDIFDBManager::storeMicroRocConfiguration(Asic* itMR,unsigned char* ConfigMR)
{
  unsigned long long mask0;
  unsigned long long mask1;
  unsigned long long mask2;
