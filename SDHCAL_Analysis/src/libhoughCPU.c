...
void cleanUI1CPU(int maxblock,int maxthread,unsigned int *d_hough,unsigned long *d_hough_layer,unsigned int *d_hough_map,houghLimits hl)
{
...
void fillHoughCPU(int maxblock,int maxthread,short *d_val,unsigned int* d_layer,unsigned int* d_hough,unsigned long* d_hough_layer,unsigned int* d_hough_map,houghLimits hl)
{
  // shared memory
...
void localRegressionCPU(float xp,float yp,float* d,float* res)
{
  ...
  //  double yi=1./2./b;
  if (phi<0) phi+=2*M_PI;
  if (xp>0 && yp>0 && phi>M_PI) phi-=M_PI;
  if (xp<0 && yp>0 && phi>M_PI) phi-=M_PI;
  if (xp<0 && yp<0 && phi<M_PI) phi+=M_PI;
  if (xp>0 && yp<0 && phi<M_PI) phi+=M_PI;
...
for (int b_idx=0;b_idx<maxblock;b_idx++)  for (int t_idx=0;t_idx<maxthread;t_idx++) {{ //STARTOFLOOP
  ...
  //printf("ith=%d ir=%d theta/r %f,%f  (z,x) %f %f \n",ith,ir,theta,r,d_x[is],d_y[is]); 
  if (ir>=0 && ir<nbinrho) 
    {
...
void ListHoughPointCPU(int maxblock,int maxthread,unsigned int* d_hough,unsigned long* d_hough_layer,unsigned int min_val,unsigned int min_layer,unsigned int* d_cand,houghLimits hl)
{
  const unsigned int nbinrho=hl.nrho;//int(limits[5]);
  const unsigned int nbintheta=hl.ntheta;//int(limits[4]);
  ...
  if (d_val[ib*nbintheta+ith]==ir )
    {
      int iwm=ib/32;
      int ibm=ib%32;
      //printf(" Copying stub %d bit %d %d %x  iwm %d ibm %d \n",ib,ith,ir,d_temp[iwm],iwm,ibm);
      if (!(d_temp[iwm] & (1<<ibm)))
	{
...
void createHoughCPU(houghParam* p,uint32_t max_stub,uint32_t max_theta,uint32_t max_rho)
{
  ...
  p->max_rho=max_rho;
   ...
   p->d_hough_layer=(unsigned long*)malloc(max_theta*max_rho*sizeof(unsigned long) );
...
void copyHoughLayerCPU(houghParam* p,unsigned long* h_hough)
{
  memcpy(h_hough, p->d_hough_layer,p->ntheta*p->nrho*sizeof(unsigned long));
...
void processHoughCPU(houghParam* p,unsigned int min_cut,unsigned int min_layer,unsigned int mode)
{
 houghLimits hl;
  ...
  //memset(p->d_hough_layer,0,p->max_theta*p->max_rho*sizeof(unsigned long));
  //printf("Number of stubs %d  grid1 %d threads1 %d \n",p->nstub,grid1,threads1);
  //getchar();
  if (mode==0)
    {
      computeHoughPointCPU(grid1, threads1,p->d_z,p->d_x,p->d_val,hl);
    }
  else
    if (mode==1)
      computeHoughPointCPU(grid1,threads1,p->d_z,p->d_y,p->d_val,hl);
