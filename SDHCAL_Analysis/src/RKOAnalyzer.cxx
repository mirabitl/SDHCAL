...
RKOAnalyzer::RKOAnalyzer(DHCalEventReader* r,DCHistogramHandler* h) 
{reader_=r;handler_=h;}

void RKOAnalyzer::initJob()
{
...
void RKOAnalyzer::initHistograms()
{
...
////////////////////////////////////
	...
	handler_->BookTH1(name.str(),MaxTriggerHandeledValue,0.1,MaxTriggerHandeledValue+0.1);	
...
/////////////////////////////////////////////		
	...
	handler_->BookTH1(name.str(),MaxTriggerHandeledValue,0.,MaxTriggerHandeledValue);	
...
////////////////////////////////////  
 for (int i=0;i<rhcol->getNumberOfElements();i++){
	...
	int difid = hit->getCellID0()&0xFF;
      ...
      if (handler_->GetTH2(name.str())!=NULL) continue;
		handler_->BookTH2(name.str(),128.,0.,128.);
  }	
  
 */ 
}

void RKOAnalyzer::RKPadConvert(int asicid,int ipad, int &i, int &j, int asicType)
{

...
void RKOAnalyzer::Clusteriser(float* HitX, float* HitY, int nHit, float* ClustX, float* ClustY, float* nHitInClust, int &nClust)
{
	int CntClust=0;
	bool Use[nHit];
...
void RKOAnalyzer::ComputeOnlineEfficiencyWindowsIntegrated(LCCollection* rhcol)
{
	...
	if((PresenceHit)&&(TriggerForEff<MaxTriggerHandeledValue)){ //security if to avoid problems
		...
		TH1* T_Eff_Th0 = handler_->GetTH1("Quality/Level0/OnlineTimeEfficiency");
		if (T_Eff_Th0==NULL)
			T_Eff_Th0 = handler_->BookTH1("Quality/Level0/OnlineTimeEfficiency",MaxTriggerHandeledValue,0.1,MaxTriggerHandeledValue+0.1);
	  
		TH1* T_Eff_Th1= handler_->GetTH1("Quality/Level1/OnlineTimeEfficiency");
		if (T_Eff_Th1==NULL)
			T_Eff_Th1 = handler_->BookTH1("Quality/Level1/OnlineTimeEfficiency",MaxTriggerHandeledValue,0.1,MaxTriggerHandeledValue+0.1);
...
void RKOAnalyzer::ComputeOnlineHitMultiplicity(LCCollection* rhcol)
{
	//Evaluate event in/off Time Windows
	bool PresenceHit = false;
	int CntHit0 = 0;
	...
	if(CntHit0){
		...
		TH1* T_TiggerMult_Th0 = handler_->GetTH1("Quality/Level0/OnlineInTimeTriggerMult");
		if (T_TiggerMult_Th0==NULL)
			T_TiggerMult_Th0 = handler_->BookTH1("Quality/Level0/OnlineInTimeTriggerMult",MaxTriggerHandeledValue,0.1,MaxTriggerHandeledValue+0.1);
	...
	if(CntHit1){
		...
		TH1* T_TiggerMult_Th1= handler_->GetTH1("Quality/Level1/OnlineInTimeTriggerMult");
		if (T_TiggerMult_Th1==NULL)
			T_TiggerMult_Th1 = handler_->BookTH1("Quality/Level1/OnlineInTimeTriggerMult",MaxTriggerHandeledValue,0.1,MaxTriggerHandeledValue+0.1);
		T_TiggerMult_Th1->SetBinContent(TriggerForMult, CntHit1);
		
	}	 
}

void RKOAnalyzer::ComputeOnlineHitMultiplicityWindowsIntegrated(LCCollection* rhcol)
{
	//Evaluate event in/off Time Windows
	bool PresenceHit = false;
	int CntHit0 = 0;
	...
	if((PresenceHit)&&(TriggerForMultInteg<MaxTriggerHandeledValue)){ //security if to avoid problems
		...
		TH1* T_TiggerMult_Th0 = handler_->GetTH1("Quality/Level0/OnlineInTimeTriggerMultWinIntegrated");
		if (T_TiggerMult_Th0==NULL)
			T_TiggerMult_Th0 = handler_->BookTH1("Quality/Level0/OnlineInTimeTriggerMultWinIntegrated",MaxTriggerHandeledValue,0.1,MaxTriggerHandeledValue+0.1);
	  
		TH1* T_TiggerMult_Th1= handler_->GetTH1("Quality/Level1/OnlineInTimeTriggerMultWinIntegrated");
		if (T_TiggerMult_Th1==NULL)
			T_TiggerMult_Th1 = handler_->BookTH1("Quality/Level1/OnlineInTimeTriggerMultWinIntegrated",MaxTriggerHandeledValue,0.1,MaxTriggerHandeledValue+0.1);
...
void RKOAnalyzer::Hit_Maps(LCCollection* rhcol)
{
	// Loop on all frames
	std::stringstream name("");
    ...
    for (unsigned int i=0;i<curMaps.size();i++) 
		handler_->GetTH2(curMaps[i])->Reset();
...
void RKOAnalyzer::ComputeOnlineClusterMultiplicity(LCCollection* rhcol)
{
	int nHit=0;
	// Loop on all frames
	for (int i=0;i<rhcol->getNumberOfElements();i++)
	...
	TH1* T_MultClust_Th0 = handler_->GetTH1("Quality/Level0/OnlineInTimeTriggerMultCluster");
	if (T_MultClust_Th0==NULL)
			T_MultClust_Th0 = handler_->BookTH1("Quality/Level0/OnlineInTimeTriggerMultCluster",MaxTriggerHandeledValue,0.,MaxTriggerHandeledValue);
...
void RKOAnalyzer::TriggerQuality(LCCollection* rhcol)
{
	...
	for(int i=0; i< nIntParameters ; i++ ){
		...
		if((GTC_Offset.find(intKeys[i])!=GTC_Offset.end())&&(DTC_Offset.find(intKeys[i])!=DTC_Offset.end())){
			...
			if (strstr(intKeys[i].c_str(),"DIF") != NULL){
				...
				TH1* hGTC = handler_->GetTH1(name.str()+"/GTC_TriggerJumps");
				if (hGTC==NULL)
					hGTC =handler_->BookTH1(name.str()+"/GTC_TriggerJumps",MaxTriggerHandeledValue,0.,MaxTriggerHandeledValue);
			
				TH1* hDTC = handler_->GetTH1(name.str()+"/DTC_TriggerJumps");
				if (hDTC==NULL)
					hDTC =handler_->BookTH1(name.str()+"/DTC_TriggerJumps",MaxTriggerHandeledValue,0.,MaxTriggerHandeledValue);
...
/*
void RKOAnalyzer::Memory_Depth(LCCollection* rhcol)
{
	...
	for(int i=0; i< nIntParameters ; i++ ){
		...
		if((GTC_Offset.find(intKeys[i])!=GTC_Offset.end())&&(DTC_Offset.find(intKeys[i])!=DTC_Offset.end())){
			...
			if (strstr(intKeys[i].c_str(),"DIF") != NULL){
				...
				TH2* hMD = handler_->GetTH2(name.str()+"/Memory_Depth");
				if (hMD==NULL)
					hMD =handler_->BookTH2(name.str()+"/Memory_Depth",128.,0.,128.);
