...
void HoughLocal::fill(std::vector<RecoPoint> *vp,uint32_t dir)
{
	thePoints_=vp;
	theVoteMax_=0;
	for (std::vector<RecoPoint>::iterator ip=thePoints_->begin();ip!=thePoints_->end();ip++)
		{
			float z=(*ip).Z();
			float x=(*ip).X();
			if (dir!=0)  x=(*ip).Y();
