...
ChamberAnalyzer::ChamberAnalyzer(DHCalEventReader* r,DCHistogramHandler* h) :trackIndex_(0),nAnalyzed_(0),clockSynchCut_(8), spillSize_(90000),maxHitCount_(500000),
...
uint32_t ChamberAnalyzer::buildVolume(IMPL::LCCollectionVec* rhcol,uint32_t seed)
{
...
void ChamberAnalyzer::processEvent()
{
  ...
  for (std::vector<uint32_t>::iterator is=vseeds.begin();is!=vseeds.end();is++)
	...
	{
	  printf(" Une trace sans doute %d hits near\n",vrh1.size());
	  continue;
	}
	...
	for (uint32_t i=0;i<96;i++)
      ...
      if (sqrt((ish.lambda[0]+ish.lambda[1])/ish.lambda[2])<0.1) continue;
      printf(" Second step %d Hits Composantes principales %f %f %f => %f \n",vrh.size(),ish.lambda[0],ish.lambda[1],ish.lambda[2], sqrt((ish.lambda[0]+ish.lambda[1])/ish.lambda[2]));
...
void ChamberAnalyzer::TracksBuilder()
{
  allpoints_.clear();
...
int32_t ChamberAnalyzer::fillTrackTable(uint32_t npoint,double ax,double bx,double ay,double by,double chi2)
{
  std::string start="(?,";
  if (useMysql_) start="(NULL,";
  std::stringstream s;
...
int32_t ChamberAnalyzer::fillPointTable(uint32_t trackid,uint32_t plan,double z,double xext,double yext,double xclus,double yclus)
{
  std::string start="(?,";
  if (useMysql_) start="(NULL,";
  std::stringstream s;
...
void ChamberAnalyzer::draw(std::vector<RecoCandTk> &tracks,std::vector<RecoPoint> &points)
{
...
void ChamberAnalyzer::newHT3(array3D<unsigned char> &cores)
{
  TH1F* hw = (TH1F*) rootHandler_->GetTH1("weights");
  if (hw==NULL)
    {
      //hhtx =(TH2F*)rootHandler_->BookTH2("HoughTransform",nhtheta,0,TWOPI,nhr,rmin,rmax);
  ...
  */
  ...
  HoughLocal htl(0,PI,-150.,150,8,8);
  htl.clear();
  htl.fill(&allpoints_);
  ...
  if (hhtx==NULL)
    {
      hhtx =(TH2F*)rootHandler_->BookTH2("HoughTransform",nhtheta,0,PI,nhr,rmin,rmax);
  ...
  for (uint32_t ith=0;ith<nhtheta;ith++)
      ...
      for (uint32_t ir=0;ir<nhr;ir++)
	{
	  ...
	  float theta=(th)*PI/nhtheta;
	  float r=rb*rbin+rmin;
	  float a=-1./tan(theta);
	  float b=r/sin(theta);
	  if ((theta<20./180*PI) || (theta>160./180.*PI)) continue;
	  printf("%f %f ===> %f %f \n",theta,r,a,b);
...
#endif
void ChamberAnalyzer::newHT2(array3D<unsigned char> &cores)
{
#ifdef USE_OPENCV
  const uint32_t nbz=60;
  const uint32_t nbx=96;
...
void ChamberAnalyzer::track2Db(std::vector<RecoCandTk> &tracks,std::vector<RecoPoint> &points)
{
