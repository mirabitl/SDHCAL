...
#define posError 0.5
PlaneCluster::PlaneCluster(RecoHit *h) : valid_(true)
{
	hits_.clear();
	hits_.push_back(h);
	//  this->calcPos();
...
bool PlaneCluster::Append(RecoHit* h)
{	
	bool append=false;
	for (std::vector<RecoHit*>::iterator it= hits_.begin();it!=hits_.end();it++)
	{
...
double PlaneCluster::X(){calcPos();return x_;}
double PlaneCluster::Y(){calcPos();return y_;}
