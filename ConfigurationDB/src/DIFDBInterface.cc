...
void DIFDBInterface::dumpAsicMap()
{
  for (std::map<uint32_t,unsigned char*>::iterator it=theAsicKeyMap_.begin();it!=theAsicKeyMap_.end();it++)
    {
      uint32_t d,a,s;
...
void DIFDBInterface::download(std::string asicType)
{
...
uint8_t DIFDBInterface::getAsicsNumber(uint32_t difid)
{
...
uint8_t DIFDBInterface::getAsicsNumberl1(uint32_t difid)
{
  return theNumberOfAsicsMap_[difid]&0xFF;
}
uint8_t DIFDBInterface::getAsicsNumberl2(uint32_t difid)
{
  return (theNumberOfAsicsMap_[difid]>>8)&0xFF;
}
uint8_t DIFDBInterface::getAsicsNumberl3(uint32_t difid)
{
  return (theNumberOfAsicsMap_[difid]>>16)&0xFF;
}
uint8_t DIFDBInterface::getAsicsNumberl4(uint32_t difid)
{
  return (theNumberOfAsicsMap_[difid]>>24)&0xFF;
}


void DIFDBInterface::dumpToTree(std::string rootPath,std::string setupName)
{
