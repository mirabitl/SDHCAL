...
#undef DEBUGBUF
#define LINELENGTH 8
TdcMessageHandler::TdcMessageHandler(std::string directory) : _storeDir(directory)
{
  _sockMap.clear();
  _tdc[0]=NULL;
  _tdc[1]=NULL;
...
void TdcMessageHandler::parseTdcData(NL::Socket* socket,ptrBuf& p) throw (std::string)
{
  ...
  //sd<<_storeDir;
  uint16_t mezz;
  uint32_t ip_address=TdcMessageHandler::convertIP(socket->hostTo());
...
void TdcMessageHandler::processMessage(NL::Socket* socket) throw (std::string)
{
  ...
  uint64_t id=( (uint64_t) TdcMessageHandler::convertIP(socket->hostTo())<<32)|socket->portTo();
