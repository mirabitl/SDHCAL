...
int main(int argc, char** argv ){
  ...
  DCBufferReader::setDAQ_BC_Period(0.4);
  std::cout<<"BC period"<<DCBufferReader::getDAQ_BC_Period()<<std::endl;
  char* FILEN ;
  char* FILEO;
  int runNumber=0 ;
  int evtNumber=0 ;
...
#define CH123
#ifdef TOMUVOL
  dher.difInChamber(26,1);
  dher.difInChamber(16,1);
  dher.difInChamber(23,1);
  dher.difInChamber(2,2);
  dher.difInChamber(13,2);
  dher.difInChamber(24,2);
  dher.difInChamber(22,3);
  dher.open(name);
#endif
#ifdef CH123
  dher.difInChamber(156,1);
  dher.difInChamber(111,1);
  dher.difInChamber(110,1);
  dher.difInChamber(51,2);
  dher.difInChamber(56,2);
  dher.difInChamber(109,2);
  dher.difInChamber(155,3);
  dher.difInChamber(57,3);
  dher.difInChamber(52,3);
  dher.open(name);
#endif
...
#undef USESTREAM
#ifdef USESTREAM
  FilterAnalyzer *a= new FilterAnalyzer( &dher,&rootHandler);
...
//     }
  ...
  while (ier>0)
	...
	std::cout<<"event "<<ier<<std::endl;
      ...
      //:wqLCTOOLS::dumpEventDetailed( dher.getEvent() ) ;
      //  getchar();
  try {
    ...
    // dher.analyzeEvent();
      // getchar();
      dher.parseRawEvent(0);
      //      getchar();
      dher.flagSynchronizedFrame();
      IMPL::LCEventImpl* evt_=dher.getEvent();
      IMPL::LCCollectionVec* HitVec=dher.createRawCalorimeterHits(false);
      if (ier%100==0)
