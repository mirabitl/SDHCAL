...
void HVCaenInterface::SetVoltageRampUp(uint32_t channel,float v0)
{
  char ParName[16];
...
std::string HVCaenInterface::GetName(uint32_t channel)
{
  char ParName[16];
...
float HVCaenInterface::GetVoltageRampUp(uint32_t channel)
{
  char ParName[16];
  sprintf(ParName,"%s","RUp");
  float param[1];
